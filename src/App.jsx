...
export default function App() {
  ...
  // --- LÓGICA DE HISTORIAL PARA GRÁFICAS ---
  const obtenerDatosGrafica = (nombreEjercicio) => {
    return historial
      ...
      .map(sesion => {
        ...
        return {
          fecha: sesion.fecha.split(',')[0], // Solo la fecha sin la hora
          peso: parseFloat(datosEj.peso),
          reps: parseInt(datosEj.reps)
        };
      })
      .reverse(); // Orden cronológico (del más viejo al más nuevo)
  };
    ...
    <div className="relative min-h-screen text-slate-100 font-sans overflow-x-hidden bg-black">
      ...
      <div className="relative z-10 flex flex-col items-center p-4 min-h-screen">
        ...
        <main className="w-full max-w-md">
          ...
          {/* PANTALLA: MOSTRAR RUTINA (Con lógica de bloqueo/desbloqueo) */}
              ...
              <div className="sticky top-0 z-50 bg-black/90 backdrop-blur-xl p-5 border border-white/10 rounded-[2.5rem] mb-4 shadow-2xl">
                <div className="flex justify-between items-center mb-5 px-1">
                  ...
                  <div className="flex items-center gap-3">
                    <button onClick={() => { if(confirm("¿Borrar rutina?")) { setRutina(null); setPantalla("seleccionDias"); } }} className="text-[10px] font-black text-red-500 uppercase border-2 border-red-500/30 px-4 py-2 rounded-xl bg-red-500/10">🗑 Borrar Plan</button>
                      ...
                      return (
                        <div key={ej.id} className={`p-4 rounded-3xl border transition-all ${done ? "bg-green-500/10 border-green-500/30 opacity-40" : "bg-white/5 border-white/10"}`}>
                          <div className="flex justify-between items-center mb-4">
                            <span onClick={() => setEjercicioDetalle(ej)} className="font-bold text-lg text-white uppercase italic cursor-pointer flex items-center gap-2">{ej.nombre} <span className="text-[9px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full border border-blue-500/30">INFO</span></span>
                            {modoEntreno && (
                              <button onClick={() => setCompletados(p => p.includes(ej.id) ? p.filter(id => id !== ej.id) : [...p, ej.id])} className={`min-w-[48px] h-12 rounded-2xl border-2 flex items-center justify-center transition-all ${done ? "bg-green-500 border-green-400 text-black" : "border-white/20 text-slate-500"}`}>{done ? "✓" : ""}</button>
                            )}
                          </div>
                          <div className="grid grid-cols-3 gap-3">
                            {['series', 'reps', 'peso'].map(campo => (
                              <div key={campo} className={`p-2 rounded-xl text-center transition-all ${modoEntreno ? "bg-white/10 border border-white/20" : "bg-black/40 opacity-60"}`}>
          ...
          {pantalla === "historial" && (
            <div className="animate-in slide-in-from-right pb-20">
              ...
              {!ejercicioGrafica ? (
                <div className="space-y-4">
                  <p className="text-[10px] font-black uppercase text-slate-500 tracking-[0.2em] mb-4">Selecciona un ejercicio para ver evolución:</p>
                  {listaEjerciciosUnicos.length === 0 ? (
                    <div className="text-center py-20 border-2 border-dashed border-white/10 rounded-[3rem] opacity-50 italic">No hay datos guardados todavía. ¡Empieza a entrenar!</div>
                  ) : (
                ...
                <div className="animate-in zoom-in duration-300">
                  <button onClick={() => setEjercicioGrafica(null)} className="text-orange-400 font-black text-[10px] mb-6 uppercase tracking-widest">← Volver a la lista</button>
                  <div className="bg-black/80 border border-orange-500/20 p-6 rounded-[2.5rem] backdrop-blur-xl shadow-2xl">
                    <h3 className="text-xl font-black italic text-white uppercase mb-8 text-center border-b border-white/10 pb-4">{ejercicioGrafica}</h3>
                    
                    <div className="h-64 w-full">
                      <ResponsiveContainer width="100%" height="100%">
                        <LineChart data={obtenerDatosGrafica(ejercicioGrafica)}>
                          <CartesianGrid strokeDasharray="3 3" stroke="#333" vertical={false} />
                          <XAxis dataKey="fecha" stroke="#666" fontSize={10} tickLine={false} axisLine={false} />
                          <YAxis stroke="#666" fontSize={10} tickLine={false} axisLine={false} />
                          <Tooltip 
                            contentStyle={{ backgroundColor: '#111', border: '1px solid #333', borderRadius: '15px', fontSize: '12px' }}
                            itemStyle={{ fontWeight: 'bold' }}
                          />
                          <Legend iconType="circle" />
                          <Line type="monotone" dataKey="peso" name="Peso (kg)" stroke="#f97316" strokeWidth={4} dot={{ r: 6, fill: '#f97316' }} activeDot={{ r: 8 }} />
                          <Line type="monotone" dataKey="reps" name="Reps" stroke="#3b82f6" strokeWidth={2} strokeDasharray="5 5" dot={{ r: 4 }} />
                        </LineChart>
                      </ResponsiveContainer>
                    </div>

                    <div className="mt-8 space-y-3">
                      <p className="text-[10px] font-black uppercase text-slate-500 text-center tracking-widest">Registros Recientes</p>
                      {obtenerDatosGrafica(ejercicioGrafica).slice().reverse().map((d, i) => (
                        <div key={i} className="flex justify-between items-center bg-white/5 p-4 rounded-2xl border border-white/5">
                          <span className="text-xs font-bold text-slate-400 italic">{d.fecha}</span>
                          <span className="font-black text-white">{d.peso}kg <span className="text-orange-400/50">x</span> {d.reps}</span>
                        </div>
                      ))}
                    </div>
                  </div>
          ...
          {/* LAS DEMÁS PANTALLAS (ANATOMIA, SELECCION DÍAS...) SE MANTIENEN IGUAL */}
          {/* ... */}
        </main>
      </div>

      {/* MODAL DETALLE GIF (Se mantiene igual) */}
      {ejercicioDetalle && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-sm rounded-[3rem] p-8 animate-in zoom-in">
            ...
            <div className="w-full aspect-square bg-white/5 rounded-[2rem] overflow-hidden mb-6 border border-white/10">
              {ejercicioDetalle.gif && <img src={ejercicioDetalle.gif} className="w-full h-full object-cover" key={ejercicioDetalle.gif} />}
            </div>
            <p className="text-slate-300 text-sm italic text-center mb-8">"{ejercicioDetalle.instrucciones}"</p>
