    ...
    <div className="relative min-h-screen text-slate-100 font-sans overflow-x-hidden bg-black">
        ...
        <main className="w-full max-w-md">
          {pantalla === "menu" && (
            <div className="flex flex-col gap-6 animate-in fade-in zoom-in duration-500">
              <button onClick={() => rutina ? setPantalla("mostrarRutina") : setPantalla("seleccionDias")} className="relative w-full py-8 border-2 border-white/20 rounded-[40px] bg-white/5 backdrop-blur-sm active:scale-95 transition-all">
                <span className="text-2xl font-black italic uppercase tracking-[0.2em]">Mi Rutina</span>
              </button>
              <button onClick={() => setPantalla("anatomia")} className="relative w-full py-8 border-2 border-white/20 rounded-[40px] bg-white/5 backdrop-blur-sm active:scale-95 transition-all">
                <span className="text-2xl font-black italic uppercase tracking-[0.2em]">Anatomía</span>
              </button>
              <button onClick={() => setPantalla("historial")} className="relative w-full py-8 border-2 border-orange-400/30 rounded-[40px] bg-orange-500/5 backdrop-blur-sm active:scale-95 transition-all">
                <span className="text-2xl font-black italic uppercase tracking-[0.2em] text-orange-400">Progreso</span>
              </button>
            </div>
          )}
          ...
          {pantalla === "seleccionDias" && (
            <div className="bg-black/60 border border-white/10 p-8 rounded-[2.5rem] backdrop-blur-3xl animate-in slide-in-from-bottom-10">
              ...
              <div className="grid grid-cols-2 gap-3 mb-8">
                {DIAS_SEMANA.map(dia => (
                  <button key={dia} onClick={() => toggleDia(dia)} className={`py-4 rounded-2xl font-bold uppercase text-[10px] ${diasSeleccionados.includes(dia) ? "bg-blue-600 shadow-lg text-white" : "bg-white/5 text-slate-400 border border-white/5"}`}>
                    {dia}
                  </button>
                ))}
              </div>
              ...
              <button onClick={() => setPantalla("menu")} className="w-full mt-4 text-[10px] font-black text-slate-500 uppercase text-center">Volver</button>
            </div>
          )}
          ...
          {pantalla === "mostrarRutina" && rutina && (
            <div className="space-y-6 animate-in fade-in pb-20">
              <div className="sticky top-0 z-50 bg-black/90 backdrop-blur-xl p-5 border border-white/10 rounded-[2.5rem] mb-4 shadow-2xl">
                <div className="flex justify-between items-center mb-5 px-1">
                  ...
                  <div className="flex items-center gap-3">
                    <button 
                      ...
                      className="text-[10px] font-black text-red-500 uppercase border-2 border-red-500/30 px-4 py-2 rounded-xl bg-red-500/10 active:scale-95 transition-all"
                    >
                      🗑 Borrar Plan
                    </button>
                ...
                <button onClick={modoEntreno ? finalizarEntrenamiento : () => setModoEntreno(true)} className={`w-full py-4 rounded-[1.5rem] font-black transition-all uppercase italic ${modoEntreno ? "bg-red-500 text-white shadow-red-500/20" : "bg-green-500 text-black shadow-green-500/20"}`}>
                ...
                <div key={dia} className="bg-black/40 border border-white/10 p-6 rounded-[2.5rem] backdrop-blur-md mb-6 shadow-xl">
                  ...
                  <div className="space-y-4">
                    {ejercicios.map((ej) => {
                      ...
                      return (
                        <div key={ej.id} className={`p-4 rounded-3xl border transition-all duration-300 ${done ? "bg-green-500/10 border-green-500/30 opacity-40" : "bg-white/5 border-white/10"}`}>
                          <div className="flex justify-between items-center mb-4">
                            <span onClick={() => setEjercicioDetalle(ej)} className="font-bold text-lg text-white uppercase italic tracking-tighter cursor-pointer flex items-center gap-2">
                              {ej.nombre} <span className="text-[9px] bg-blue-500/20 text-blue-400 px-2 py-0.5 rounded-full border border-blue-500/30 uppercase">INFO</span>
                            </span>
                            {modoEntreno && (
                          ...
                          <div className="grid grid-cols-3 gap-3">
                            {/* Inputs ahora se activan SOLO en modoEntreno */}
                            <div className={`p-2 rounded-xl text-center transition-all ${modoEntreno ? "bg-white/10 border border-white/20" : "bg-black/40 opacity-60"}`}>
                              ...
                              <input 
                                ...
                                className="bg-transparent w-full text-center font-bold outline-none text-white disabled:cursor-not-allowed" 
                              />
                            </div>
                            <div className={`p-2 rounded-xl text-center transition-all ${modoEntreno ? "bg-white/10 border border-white/20" : "bg-black/40 opacity-60"}`}>
                            ...
                            <div className={`p-2 rounded-xl text-center border transition-all ${modoEntreno ? "bg-blue-600/30 border-blue-500 shadow-[0_0_10px_rgba(59,130,246,0.2)]" : "bg-blue-900/20 border-blue-500/30 opacity-60"}`}>
                              ...
                              <input 
                                ...
                                className="bg-transparent w-full text-center font-bold outline-none text-blue-300 disabled:cursor-not-allowed" 
                              />
                            </div>
                          </div>
                        </div>
                ...
                <div className="space-y-4 animate-in zoom-in duration-300">
                  ...
                  <div className="bg-black/60 border border-white/10 p-6 rounded-[2rem] backdrop-blur-xl">
                    ...
                    {BASE_EJERCICIOS[musculoSeleccionado.toLowerCase()]?.map((ej, i) => (
                      <div key={i} className="border-l-4 border-blue-500 p-4 bg-white/5 mb-3 rounded-r-2xl flex justify-between items-center text-white">
                  ...
                  <div key={item.id} className="bg-black/40 backdrop-blur-md p-6 rounded-[2rem] border border-white/5 mb-4 shadow-lg">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {item.ejercicios.map((e, idx) => (
                        <span key={idx} className="text-[9px] text-slate-400 bg-white/5 px-2 py-1 rounded-lg border border-white/5">{e.nombre} - {e.peso}kg</span>
                      ))}
                    </div>
                  </div>
                ))
      ...
      {ejercicioDetalle && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
          ...
          <div className="relative bg-[#0a0a0a] border border-white/10 w-full max-w-sm rounded-[3rem] p-8 animate-in zoom-in shadow-2xl">
            <h3 className="text-2xl font-black italic text-blue-500 uppercase mb-6 text-center">{ejercicioDetalle.nombre}</h3>
            <div className="w-full aspect-square bg-white/5 rounded-[2rem] overflow-hidden mb-6 border border-white/10 flex items-center justify-center">
              {ejercicioDetalle.gif ? (
                ...
                <div className="text-center p-6 text-slate-500"><p className="text-4xl">🏋️‍♂️</p></div>
              )}
            </div>
            <div className="space-y-2 mb-8 text-center">
              <p className="text-[10px] font-black uppercase text-blue-500/50 tracking-widest">Técnica</p>
              <p className="text-slate-300 text-sm italic">"{ejercicioDetalle.instrucciones || "Consulta la técnica con un profesional."}"</p>
            </div>
